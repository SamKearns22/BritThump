...
function renderLayout(title, bodyHtml, extraHead = '') {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>${escapeHtml(title)} — The Daily Wind-Up</title>
...
function renderAdmin(posts, message, errorMessage) {
  const postRows = posts.map(post => `
    <li class="admin-post-row">
      <a href="/article/${post.slug}" target="_blank">${escapeHtml(post.title)}</a>
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/style.css') {
      return serveStaticFile(res, path.join(PUBLIC_DIR, 'style.css'), 'text/css');
    }
...
server.listen(PORT, () => {
  console.log(`The Daily Wind-Up is running at http://localhost:${PORT}`);
